...
const TourCarousel = ({ categories }) => {
    ...
    const items = categories.flatMap(({ category, id }) => {
        const categoryData = tourData.find(tourCategory => tourCategory.category === category);
    ...
    return (
        <>
            <div className={styles.principalTitle}>
                <FaSearchLocation className={styles.IconTitle} />
                <h3 className={styles.firstTitle}>{t('Cards.title')}</h3>
                <p className={styles.principalSubtitulo}>
                    {t('Cards.subtitle')}
                </p>
            </div>
                        ...
                        <div key={index} className={styles.card}>
                            ...
                            {item.img && (
                                <div className={styles.imageContainer}>
                                    <img src={item.img} alt={item.text} className={styles.cardImage} />
                                    <div className={styles.iconOverlay}>
                                        {item.icons?.map((iconId) => {
                                            const iconData = iconList.find((icon) => icon.id === iconId);
                                            return iconData ? (
                                ...
                                <div className={styles.cardTextContent}>
                                    <div className={styles.CardDetails}>
                                        ...
                                        <div className={styles.CardDetailsFinal}>
                                            <p className={styles.CardDetailsLista}><GiNetworkBars className={styles.CardDetailsIcon} />{item.nivel}</p>
                                            <p className={styles.CardDetailsLista}><GrGroup className={styles.CardDetailsIcon} />{item.grupo}</p>
                                        </div>
                                    </div>
                                </div>
